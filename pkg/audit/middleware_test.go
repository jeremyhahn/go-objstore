...
// newBufferedAuditLogger returns an audit logger writing JSON records into buf.
func newBufferedAuditLogger(buf *bytes.Buffer) AuditLogger {
...
// TestAuditMiddlewareSharedInboundRequestID verifies that the audit middleware
// reuses the request ID established by RequestIDMiddleware (here: a sanitized
// inbound X-Request-ID) instead of generating a second ID, and does not
// overwrite the X-Request-ID response header.
func TestAuditMiddlewareSharedInboundRequestID(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	router.Use(AuditMiddleware(auditLogger))
	router.GET("/objects/test-key", func(c *gin.Context) {
		c.Status(http.StatusOK)
	})

	const inboundID = "client-supplied-id-123"
	req := httptest.NewRequest(http.MethodGet, "/objects/test-key", nil)
	req.Header.Set(middleware.RequestIDHeader, inboundID)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)

	if got := w.Header().Get(middleware.RequestIDHeader); got != inboundID {
		t.Errorf("X-Request-ID response header = %q, want %q (audit middleware must not overwrite it)", got, inboundID)
	}
	if output := buf.String(); !strings.Contains(output, inboundID) {
		t.Errorf("audit record does not contain request ID %q: %s", inboundID, output)
	}
}

// TestAuditMiddlewareSharedGeneratedRequestID verifies that when no inbound
// X-Request-ID is supplied, the audit record carries the ID generated by
// RequestIDMiddleware (the one returned in the response header), not a second
// independently generated ID.
func TestAuditMiddlewareSharedGeneratedRequestID(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	if headerID == "" {
		t.Fatal("expected RequestIDMiddleware to set the X-Request-ID response header")
	}
	if output := buf.String(); !strings.Contains(output, headerID) {
		t.Errorf("audit record does not contain the shared request ID %q: %s", headerID, output)
	}
}

// TestAuditMiddlewareStandaloneRequestID verifies that without
// RequestIDMiddleware the audit middleware still produces a request ID and
// sets the response header itself.
func TestAuditMiddlewareStandaloneRequestID(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	headerID := w.Header().Get(middleware.RequestIDHeader)
	if headerID == "" {
		t.Fatal("expected audit middleware to set the X-Request-ID response header")
	}
	if output := buf.String(); !strings.Contains(output, headerID) {
		t.Errorf("audit record does not contain request ID %q: %s", headerID, output)
	}
}
...
func TestAuditUnaryInterceptor(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setupCtx: func(ctx context.Context) context.Context {
				return context.WithValue(ctx, adapters.PrincipalContextKey{}, &adapters.Principal{
					ID:   "user123",
					Name: "john.doe",
				})
			},
...
// TestAuditMiddlewarePrincipalPointer is a regression test for H1: the REST
// authentication middleware stores *adapters.Principal (pointer) under the
// "principal" gin context key. The audit middleware must handle the pointer
// form and correctly populate UserID/Principal in the audit record.
func TestAuditMiddlewarePrincipalPointer(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	router := gin.New()
	router.Use(AuditMiddleware(auditLogger))
	router.GET("/objects/testkey", func(c *gin.Context) {
		// Simulate what AuthenticationMiddleware does: stores *adapters.Principal
		c.Set("principal", &adapters.Principal{
			ID:   "ptr-user",
			Name: "ptr-name",
		})
	...
	req := httptest.NewRequest(http.MethodGet, "/objects/testkey", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)

	output := buf.String()
	if output == "" {
		t.Fatal("expected audit log output, got none")
	}
}

// TestAuditUnaryInterceptorPrincipalTypedKey is a regression test for H1: the
// gRPC authentication interceptor now stores *adapters.Principal under
// adapters.PrincipalContextKey{}. The audit interceptor must extract it.
func TestAuditUnaryInterceptorPrincipalTypedKey(t *testing.T) {
	var buf bytes.Buffer
	config := &Config{
		Enabled: true,
		Format:  FormatJSON,
	...
	// Simulate what gRPC auth interceptor now does
