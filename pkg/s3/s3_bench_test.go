...
// benchS3Client is a mock S3 client for benchmarking
...
func BenchmarkS3AddPolicy(b *testing.B) {
	storage := newMockS3()
...
func BenchmarkS3GetPolicies(b *testing.B) {
	storage := newMockS3()
...
func BenchmarkS3PutGetDelete(b *testing.B) {
	storage := newMockS3()
	data := bytes.NewReader(make([]byte, 1024)) // 1KB
