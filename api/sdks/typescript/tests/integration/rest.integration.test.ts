...
describe('REST Integration Tests', () => {
  ...
  describe('Object Operations', () => {
    it('should put an object', async () => {
      const response = await client.put({
        ...
        metadata: {
          ...
          custom: { author: 'integration-test' },
        },
      });
    ...
    it('should update object metadata', async () => {
      const response = await client.updateMetadata({
        ...
        metadata: {
          contentType: 'text/plain',
          custom: { author: 'updated-test', version: '2.0' },
        },
      });
    ...
    it('should list objects', async () => {
      const response = await client.list({ prefix: 'test-' });
  ...
  describe('Replication Policies', () => {
    ...
    it('should add a replication policy', async () => {
      try {
        ...
        // Local backend doesn't support replication
        expect(error).toBeDefined();
      }
    });
  ...
  describe('Error Handling', () => {
    it('should handle getting a nonexistent object', async () => {
      const nonexistentKey = `nonexistent-${Date.now()}.txt`;
  ...
  describe('Archive Operations', () => {
    const archiveKey = `archive-${Date.now()}.txt`;
