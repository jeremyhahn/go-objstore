...
describe('gRPC Integration Tests', () => {
  ...
  const testKey = `test-grpc-${Date.now()}.txt`;
  const testData = Buffer.from('Hello from gRPC!');
  ...
  describe('Object Operations', () => {
    it('should put an object', async () => {
      const response = await client.put({
        ...
        metadata: {
          ...
          custom: { author: 'grpc-integration-test' },
        },
      });
    ...
    it('should update object metadata', async () => {
      const response = await client.updateMetadata({
        ...
        metadata: {
          contentType: 'text/plain',
          custom: { author: 'updated-grpc-test', version: '2.0' },
        },
      });
    ...
    it('should list objects', async () => {
      const response = await client.list({ prefix: 'test-grpc-' });
  ...
  describe('Error Handling', () => {
    it('should handle getting a nonexistent object', async () => {
      const nonexistentKey = `nonexistent-grpc-${Date.now()}.txt`;
  ...
  describe('Lifecycle Policies', () => {
    const policyId = `policy-grpc-${Date.now()}`;
  ...
  describe('Replication Policies', () => {
    const replicationId = `replication-grpc-${Date.now()}`;
  ...
  describe('Archive Operations', () => {
    const archiveKey = `archive-grpc-${Date.now()}.txt`;
