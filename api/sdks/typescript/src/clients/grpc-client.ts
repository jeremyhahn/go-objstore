      ...
      this.client.getReplicationPolicies({}, this.callMeta, (error: grpc.ServiceError | null, response: ProtoGetReplicationPoliciesResponse) => {
        ...
        resolve({
          policies: response.policies.map((p: ProtoReplicationPolicy) => ({
            ...
            destinationSettings: p.destinationSettings,
            checkIntervalSeconds: Number(p.checkIntervalSeconds),
        ...
        resolve({
          ...
          result: response.result
            ? {
                ...
                synced: Number(response.result.synced),
                deleted: Number(response.result.deleted),
                failed: Number(response.result.failed),
                bytesTotal: Number(response.result.bytesTotal),
                durationMs: Number(response.result.durationMs),
        ...
        resolve({
          ...
          status: response.status
            ? {
                ...
                totalObjectsSynced: Number(response.status.totalObjectsSynced),
                totalObjectsDeleted: Number(response.status.totalObjectsDeleted),
                totalBytesSynced: Number(response.status.totalBytesSynced),
                totalErrors: Number(response.status.totalErrors),
                lastSyncTime: response.status.lastSyncTime
                  ? new Date(response.status.lastSyncTime.seconds * 1000)
                  : undefined,
                averageSyncDurationMs: Number(response.status.averageSyncDurationMs),
                syncCount: Number(response.status.syncCount),
              }
            : undefined,
          message: response.message,
        });
